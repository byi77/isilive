...
7. Ergebnis B: Blizzard Settings zeigen — gruppiert in eigene Hauptsektionen mit Beta-Hinweis oben und VIP-Gast-Einstellungen unten:
   ...
   - **Sounds**: Soundkanal-Selector `Master / SFX` mit Default `Master`; `Sound: Lead Transfer`, `Sound: Full Group`, `Sound: Incoming Summon`, `Eingehende-Beschwoerung-Hinweis alle 5 Sekunden wiederholen`, `Sound: Battle Res`, `Sound: Battle Res Ready`, `Sound: Bloodlust`, `Sound: Bloodlust Ready`, `Sound: PI received`, `Sound: Ready Check Complete`, `Sound: Tank died` und `Sound: Healer died`, jeweils mit eigenem Texture-Play-Button zum Probehoeren fuer direkte Soundausgaben auch bei deaktiviertem Toggle. Die Eigen-Tod-Schalter fuer Tank und Heiler werden als eingerueckte Child-Option direkt unter dem jeweiligen Tank-/Heiler-Tod-Klanghinweis gerendert. Native WoW-Text-to-Speech-Controls werden nicht gerendert. Settings-Texte sind fuer alle acht gepflegten UI-Locale-Tabellen nachbearbeitet; andere Bereiche der vorbereiteten Locales duerfen weiterhin englische Fallbacks tragen. Eingehende Beschwoerungen des lokalen Spielers spielen den Summon-Sound ueber den klassischen `CONFIRM_SUMMON`-Pfad und ueber den statusbasierten `INCOMING_SUMMON_CHANGED`-Pfad nur bei `Enum.SummonStatus.Pending`; bei aktiviertem Loop-Schalter wird der Klang alle 5 Sekunden wiederholt, solange derselbe Live-Status fuer `player` verifiziert `Pending` bleibt. `deDE` nutzt dafuer die statische Ansage `Beschwoerung aktiv`, alle anderen Client-Locales behalten `Portal.ogg`.
