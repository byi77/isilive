...
2. Das Season-Dataset wird ueber `activeSeasonID` im normalisierten Manifest `data/isiLive_seasons.lua` ausgewaehlt; aktuell `midnight_s2` mit dem live 8-Dungeon-Midnight-Season-2-Portalpool. Portal-, LFG-Activity-, Anzeige-, Stufengate- und Portalraum-Indizes werden aus denselben Dungeon-Datensaetzen erzeugt. Bei Login und `CHALLENGE_MODE_MAPS_UPDATE` darf ein exakter, eindeutiger Blizzard-Challenge-Map-Satz nur auf Datensaetze mit `autoDetectFromChallengeMaps=true` automatisch wechseln. Beide Midnight-Seasons tragen diese Freigabe, die Auswahl bleibt runtime-only und schreibt `activeSeasonID` nie um; `activeSeasonID` bleibt der manuelle Fallback, wenn kein exakter Map-Satz matcht. `requiresForces` ist ein reines Build-Gate-Flag und beschreibt kein Runtime-Verhalten: S2 traegt `requiresForces=true` und verlangt damit eine zur Season passende generierte MDT-Forces-DB, das ebenfalls weiterhin waehlbare S1 traegt `requiresForces=false` und bleibt ohne eigene DB nutzbar. Unabhaengig davon bleibt der Blizzard-basierte Dungeon-Gesamtfortschritt immer sichtbar, waehrend Nameplate-Mobprozente, Mob-Forces-Tooltips und MDT-Gesamtwert-Fallbacks ohne exakten Season-Match, nach Ablauf von `expiresAt` oder ohne verifizierbares aktuelles Datum geschlossen bleiben.
...
7. Die Addon-Sprache kommt aus der Lokalisierungstabelle. Neue produktive Texte werden in Deutsch und Englisch gepflegt; vorbereitete weitere Locales duerfen englischen Fallback oder nachbearbeitete Community-Uebersetzungen tragen. Settings-Texte sind fuer die vorbereiteten Locales `frFR`, `esES`, `ptBR`, `itIT`, `ruRU` und `trTR` nachbearbeitet; andere Bereiche duerfen weiterhin englische Fallbacks tragen.
...
| UC-11 | M+Marker World Marker | 8 sichere World-Marker-Buttons fuer direktes Place/Clear |
| UC-12 | Roster-Panel Mini Mode | Collapse-Toggle blendet Roster-Liste und `Travel` aus, waehrend kompakte Marker- und Management-Tools sichtbar bleiben; im V-Layout ohne Tool-Ueberschriften und mit M+Markern als zwei Spalten mit je vier Markern |
...
2. Inputs: Pending-Status plus eventuell sichtbare Gruppenmetadaten und Activity-/Map-Kontext aus dem Queue-Payload.
3. Verarbeitung: Das Queue-Handling speichert nur den Kontext fuer einen gruppierten Join und darf Dungeon-/Teleport-Ziele nur aus eindeutig verifiziertem Activity-/Map-Kontext ableiten. Ein informationsloses Folgeevent ohne verifizierten Gruppennamen behaelt einen bereits erfassten Pending-Kontext; ein neuer verifizierter Gruppenname ersetzt ihn vor dem Gruppenbeitritt. Nach dem bestaetigten Beitritt gewinnt der zuvor erfasste Pending-Kontext und wird genau einmal verbraucht. Mehrdeutige `activityIDs`, fehlende Maps oder reine Namen bleiben unresolved.
4. Output: Der Pending-Kontext fuer den gruppierten Join enthaelt hoechstens den erfassten Gruppennamen.
5. Erfolgskriterium: Der gruppierte Queue-Chat kann den erfassten Gruppenkontext nutzen, und aus Queue-Events wird nur dann ein Dungeon-Ziel gesetzt, wenn die Activity-/Map-Quelle eindeutig ist.
...
3. Verarbeitung: Danach postet das Addon die Key-Zeile des lokalen Spielers in den passenden verifizierten Gruppenchat: automatische Instanzgruppen nutzen `INSTANCE_CHAT`, normale Gruppen nutzen `PARTY`, und ohne verifizierten Gruppenchat-Kanal bleibt der sichtbare Post geschlossen. Bevorzugt wird ein Blizzard-Owned-Keystone-Hyperlink (`|Hkeystone:...|h`) oder ein verifizierter Keystone-Itemlink fuer `itemID 180653` (`|Hitem:180653...|h`). Wenn kein echter Blizzard-Link verfuegbar ist, wird nur eine ungefaerbte Klartext-Zeile mit Dungeon und Level genutzt; ein Fake-Hyperlink wird nicht gebaut.
...
8. Regel: Der Admin-Testmodus (`/isilive testall`) verwendet den Full-Dummy-Preview-Pfad, inklusive sichtbarem positivem Dummy-Delta, einer Ghost-/Leaver-Zeile, Demo-Daten fuer M+-Timer, Combat-CDs, unteren M+-Forces-Tracker, Statsbox, Portal-Navigator, Centerbox-Portal, Non-Mythic-Dungeon-Entry-Centerbox, M+-Forces-Nameplates/-Tooltip, LFG-Bonusmarker, Ready-Check-Hold-Zeilen, Share-Keys-Cooldown, Death-Alert-Preview, Sound-Preview und verschiebbarem Demo-Simulations-Tablet. Das Tablet ist zusaetzlich ueber `/isilive sim` umschaltbar, fuehrt nur lokale Vorschau-Hooks aus, markiert blockierte Simulationen farblich und textuell und sendet keine echten Chatposts oder Gruppenaktionen. Es dockt standardmaessig mit 12 px Abstand rechts an der tatsaechlichen Main-UI an und richtet die Oberkanten aus, soweit der Sichtbereich dies erlaubt; reichen die effektiven physischen Sichtbereichsgrenzen dort nicht, waehlt es deterministisch links, unterhalb oder oberhalb beziehungsweise verschiebt die Seitenposition nur entlang der Bildschirmkante. Solange es sichtbar und angedockt bleibt, wird diese Platzwahl nach Main-UI-Drag, Layoutgroessenwechsel, UI-Scale-Wechsel und Sichtbereichsaenderung erneut berechnet. Ein manueller Drag loest die Verankerung fuer die Sitzung und unterdrueckt diese automatischen Neuberechnungen, waehrend `Andocken` sie anhand des aktuellen Zustands wieder aktiviert. Die Aktionsoberflaeche haelt `Komplette Vorschau` und `Simulation zuruecksetzen` separat erreichbar und filtert die uebrigen lesbaren Aktionen ueber `Gruppe`, `M+`, `Warnungen` und `Extras`. Centerbox-Portal und Non-Mythic-Dungeon-Entry-Centerbox bleiben im Demomodus parallel sichtbar, damit sie sich nicht gegenseitig verdraengen. Demo-Feature-Schalter einschliesslich `Accepted-invite notice`, `Group-join target notice` und Sound-Preview-Settings werden nur temporaer gesetzt und beim Verlassen auf die vorherigen User-Settings zurueckgesetzt. Beim ersten Demo-Start nach Reload erzeugte Rosterzeilen duerfen die rosterabhaengige M+-Framehoehe nicht mit der generischen Layout-Hoehe ueberschreiben; erster und spaetere Demo-Starts bleiben gleich gross.
...
7. Ergebnis B: Blizzard Settings zeigen — gruppiert in eigene Hauptsektionen mit Beta-Hinweis oben und VIP-Gast-Einstellungen unten:
   ...
   - **Sounds**: Soundkanal-Selector `Master / SFX` mit Default `Master`; `Sound: Lead Transfer`, `Sound: Full Group`, `Sound: Incoming Summon`, `Eingehende-Beschwoerung-Hinweis alle 5 Sekunden wiederholen`, `Sound: Battle Res`, `Sound: Battle Res Ready`, `Sound: Bloodlust`, `Sound: Bloodlust Ready`, `Sound: PI received`, `Sound: Pet stuck`, `Sound: Ready Check Complete`, `Sound: Tank died` und `Sound: Healer died`, jeweils mit eigenem Texture-Play-Button zum Probehoeren fuer direkte Soundausgaben auch bei deaktiviertem Toggle. Die Eigen-Tod-Schalter fuer Tank und Heiler werden als eingerueckte Child-Option direkt unter dem jeweiligen Tank-/Heiler-Tod-Klanghinweis gerendert. Native WoW-Text-to-Speech-Controls werden nicht gerendert. Settings-Texte sind fuer alle acht gepflegten UI-Locale-Tabellen nachbearbeitet; andere Bereiche der vorbereiteten Locales duerfen weiterhin englische Fallbacks tragen. Eingehende Beschwoerungen des lokalen Spielers spielen den Summon-Sound ueber den klassischen `CONFIRM_SUMMON`-Pfad und ueber den statusbasierten `INCOMING_SUMMON_CHANGED`-Pfad nur bei `Enum.SummonStatus.Pending`; bei aktiviertem Loop-Schalter wird der Klang alle 5 Sekunden wiederholt, solange derselbe Live-Status fuer `player` verifiziert `Pending` bleibt. `deDE` nutzt dafuer die statische Ansage `Beschwoerung aktiv`, alle anderen Client-Locales behalten `Portal.ogg`.
...
Ziel: Live-BRes, Bloodlust/Heroism/Time Warp, aktive Mythic+-Timer-Cutoffs, den M+-Killtracker und gesyncten Interrupt-Cooldown-State im Roster-Panel zeigen, ohne zu raten; in verifizierten Non-Challenge-Party-Dungeons duerfen nur BRes-/Bloodlust-Anzeige und weitere ausdruecklich freigegebene Utility-Pfade aktiv bleiben.
...
6. Verarbeitung: Solange ein aktiver Mythic+-Timer laeuft und das Roster-Panel sichtbar ist, darf der One-Second-Utility-Ticker nur die betroffenen CD-, Ready- und M+-Zeilen gezielt aktualisieren und keinen vollstaendigen Roster-/Layout-Render ausloesen. Der M+-Timer besitzt keinen eigenen Frame-`OnUpdate`-Poller; jeder produktive `GetTimerData()`-Read liest die Blizzard-World-Elapsed-Zeit geschuetzt bei Bedarf und behaelt bei fehlenden, fehlerhaften, geschuetzten oder nicht-finiten Werten den letzten belastbaren Stand. In einem verifizierten `trackedPartyRun` ohne M+-Timer darf der Ticker nur den Utility-Rescan fuer BRes-/Bloodlust-Anzeige treiben. Hidden-Modus darf diesen Utility-Poller nicht weiterlaufen lassen, und beim erneuten Oeffnen der UI darf genau ein frischer Utility-Rescan nur auf dem ersten sichtbaren Render nach Dirty-Markierung stattfinden.
...
## UC-16 BR- und Bloodlust-Gruppen-Announce

Ziel: Jeder BR- und Bloodlust-Cast eines isiLive-Spielers wird im Mythic+ und in einem verifizierten `trackedPartyRun` genau einmal als lokalisierte Chat-Zeile an alle isiLive-Peers verteilt, ohne die 12.0-`ADDON_ACTION_FORBIDDEN`-Regression von `SendChatMessage` zu treffen und ohne `"table index is secret"`-Spam durch Casts anderer Spieler zu produzieren.

1. Trigger: `UNIT_SPELLCAST_SUCCEEDED` feuert mit `unit == "player"` waehrend `C_ChallengeMode.GetActiveChallengeMapID()` einen aktiven Key meldet oder `RuntimeState.IsTrackedPartyRunActive()` einen verifizierten Non-Challenge-PartyRun-Kontext meldet.
...
3. Verarbeitung: Die eigene `spellID` wird gegen `BR_SPELL_IDS` (Rebirth, Raise Ally, Intercession, Soulstone Resurrection) und `LUST_CAST_IDS` (Bloodlust, Heroism, Time Warp, Primal Rage, Fury of the Aspects, Harrier's Cry, Feral Hide Drums, Drums of Fury / Mountain / Maelstrom / Deathly Ferocity, Ancient Hysteria, Netherwinds) geprueft.
4. Regel: Ein 3-Sekunden-Dedup-Fenster pro verifiziertem `sourceName|spellID` schluckt duplizierte Cast-Events im selben Burst; bleibt der Castername unresolved, wird weder geraten noch gesendet. `CHALLENGE_MODE_START` und `CHALLENGE_MODE_COMPLETED` rufen `Reset()` und loeschen die Dedup-Map vollstaendig.
...
12. Erfolgskriterium: Ein BR- oder Lust-Cast im Mythic+ oder verifizierten `trackedPartyRun` erzeugt genau eine sichtbare Chat-Zeile pro isiLive-Empfaenger, der Self-Render beim Sender, keinen `ADDON_ACTION_FORBIDDEN`-Popup und keinen `"table index is secret"`-Fehler; Toggles entfernen die Zeile vollstaendig auf Senderseite, Peers ausserhalb eines aktiven Keys oder verifizierten `trackedPartyRun` sehen keine Zeile.
...
1. Trigger: `UNIT_HEALTH` fuer `player` oder `party1`-`party4` waehrend eines aktiven M+-Runs oder eines verifizierten `trackedPartyRun`.
2. Voraussetzung: `deathAlertEnabled` ist aktiv (Default an); ausserhalb eines aktiven Keys oder verifizierten `trackedPartyRun`, im Raidmodus oder bei deaktivierter Option bleibt der Pfad stumm.
...
5. Darstellung: Eine grosse rote rahmenlose Bildschirmwarnung mit lokalisiertem Rollentext in Grossbuchstaben (`TANK DIED` / `HEALER DIED`, auf `deDE` `TANK TOT` / `HEILER TOT`) und Scale-Punch-Animation (uebergross einfliegend, Fade-out nach rund 1,7 Sekunden). Die Audioform ist statisch: Tank-Tode spielen `TankDied.wav`, Heiler-Tode spielen `HealerDied.wav`; auf `deDE`-Clients werden stattdessen `TankDied_deDE.wav` und `HealerDied_deDE.wav` abgespielt, alle anderen Client-Locales bleiben bei den englischen Dateien. Alle Varianten laufen ueber den konfigurierten isiLive-Soundkanal. Der gezaehlte Gesamtwert erscheint im M+-Timer-Totenkopf und im aktiven M+-Killtracker direkt hinter dem Dungeon-/Keynamen als Totenkopfmarker mit roter Zahl.
...
8. Settings: `deathAlertEnabled` schaltet die DeathWatch-Erkennung und den Bildschirmtext. Tank- und Heiler-WAV-Ausgaben sind getrennt ueber `soundTankDiedEnabled` und `soundHealerDiedEnabled` schaltbar und laufen weiterhin ueber den global gewaehlt gespeicherten Soundkanal. Eigene Tank- und Heiler-Tode haben zusaetzliche Sound-Gates (`soundOwnTankDiedEnabled`, `soundOwnHealerDiedEnabled`, Default an), die als Child-Option unter dem jeweiligen Tank-/Heiler-Tod-Klanghinweis erscheinen und nur die eigene WAV-Ausgabe unterdruecken; Bildschirmtext, Death-Counter und Sounds fuer andere Gruppenmitglieder bleiben aktiv.
...
2. Voraussetzung: Der Schalter `vipBloodlustDebuffButtonWarningEnabled` ist explizit aktiv. Neue und sanitizte DBs deaktivieren ihn standardmaessig; der Schalter bleibt im VIP-Settings-Abschnitt immer sichtbar, unabhaengig von der aktuellen Klasse, und steht als normale nicht eingerueckte Option direkt unter dem Gilded-Brutosaur-Mute vor der duennen blauen DK-Trennlinie.
...
5. Button-Quelle: Markiert werden nur sichtbare Actionbar-Buttons, deren Spell-ID ueber Button-API, Action-Slot, Secure-Actionbutton-Attribut oder Macro-Spell-ID exakt als Bloodlust-Klassen- oder Pet-Spell (`2825`, `32182`, `80353`, `264667`, `390386`, `466904`, `90355`, `160452`) verifiziert ist.
6. Validierungsstand: Mage-Time-Warp-Actionbuttons wurden ingame mit Spell-ID `80353` verifiziert. Externe, nicht ingame bestaetigte Time-Warp-Varianten wie `428941` bleiben ohne Live-Observation unresolved und werden nicht aufgenommen.
7. Regel: Drums zaehlen fuer diese Warnung nicht. Icon-, Textur-, Namens-, Klassenlisten-Ratefallbacks und synthetische Button-Ziele sind verboten.
8. Darstellung: Auf jedem verifizierten sichtbaren Bloodlust-Button wird dasselbe rote Kreuz-Overlay genutzt wie bei den VIP-DK-Buttonwarnungen. Verschwindet Debuff oder Button-Ziel, wird das Overlay wieder verborgen.
9. Raid-Hard-off: Im Raidmodus wird der Eventpfad vorgelagert unterdrueckt, sodass die Warnung keinen Output erzeugt.
10. Erfolgskriterium: Nur ein verifizierter lokaler Bloodlust-Klassencharakter mit aktivem gepflegtem Erschoepfungs-/Satt-Debuff sieht ein rotes Kreuz auf eindeutig erkanntem Bloodlust-Button; Nicht-BL-Klassen, Drums, deaktiviertes Setting und unverifizierte Quellen bleiben stumm.

## UC-27 Versteckter Non-Challenge-PartyRun-Utility-Kontext

Ziel: isiLive bleibt ein M+-Tool, haelt aber ausgewaehlte Utility-Funktionen in verifizierten normalen, heroischen oder sonstigen verfolgten Non-Challenge-Party-Dungeons aktiv.
...
12. Das Verlassen oder Entferntwerden aus einer normalen Party muss den aktuellen Frame-Visibility-State behalten und ehemalige Mitglieder als Ghost-Zeilen halten, bis ein deterministischer Prune-Pfad eintritt; aktive Mitglieder muessen dabei weiterhin vor persistierten Ghosts sichtbar bleiben. Ghosts sind pro Charakternamen eindeutig, nicht pro Party-Slot, und werden erst ab fuenf aktiven Mitgliedern regulaer geprunet. Damit eine dauerhaft kleine Gruppe beim Durchwechseln von Mitgliedern nicht unbegrenzt waechst, ist die Ghost-Historie zusaetzlich hart begrenzt: es bleiben hoechstens die zehn juengsten Abgaenge erhalten, aeltere werden verworfen. Die Reihenfolge stammt aus einem monoton steigenden Stempel beim Ghost-Werden, nicht aus der Tabellen-Iterationsreihenfolge.
...
15. Raid-Gruppenerkennung bei mehr als 5 Mitgliedern blendet die Addon-UI aus, unterdrueckt Background-Processing einschliesslich hidden Kick-Keep-Alive und delayed Post-Run-Refresh-Ausfuehrung, gibt keine Raid-Transition-Notice aus und blockiert das Zurueckschalten auf M/V, bis die Gruppengroesse wieder Party ist. War die Main-UI direkt vor der Raid-Unterdrueckung sichtbar, wird sie beim Rueckweg aus dem Raid wieder geoeffnet; war sie geschlossen, bleibt sie geschlossen.
...
24. Meldet der Client per `UI_ERROR_MESSAGE` exakt den lokalisierten Text `ERR_PET_SPELL_NOPATH` ("No path available for your pet"), spielt ausserhalb des Raids sofort die statische Pet-Stuck-Sprachansage `PetStuck.wav` beziehungsweise auf `deDE` `PetStuck_deDE.wav` ("Achte auf deinen Begleiter!") ueber den gespeicherten Soundkanal. Weitere Treffer innerhalb von 5 Sekunden, andere Fehlermeldungen, Secret-Value-Payloads, eine fehlende Client-Konstante und ein deaktiviertes `soundPetStuckEnabled` bleiben stumm.
