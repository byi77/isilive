...
`tools/validate_usecases.lua` ist Pflicht fuer das Release-Gate, fuehrt beide Regelvalidatoren zuerst aus und validiert danach die aktuell registrierten Szenarien ueber `tools/usecase_scenarios.lua` (Stand nach Battle-Res-ready-Key-Start-, Battle-Res-ready-UI-Rescan-, Bloodlust-ready-Reminder-, Ready-Sound-Disarm- und Accepted-Invite-Level-Fix: 1942 Szenarien). Die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
