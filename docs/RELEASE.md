...
Der Wrapper fuehrt Style-, Lint-, Syntax-, Metrics-, Locale-, Secret-Value-, Sound-, API-, Coverage-, Regel- und Usecase-Gates in derselben Reihenfolge aus, in der lokale Release-Vorbereitung sie erwarten sollte. Letzter lokaler Audit-Stand nach dem 0.9.330-Doku-Update: `Local CI preflight passed`, `lua tools/validate_usecases.lua` mit `2089 passed, 0 failed`, Coverage `92.43%` bei Gate `>=88.00%`.
...
`tools/validate_usecases.lua` ist Pflicht fuer das Release-Gate, fuehrt beide Regelvalidatoren zuerst aus und validiert danach die aktuell registrierten Szenarien ueber `tools/usecase_scenarios.lua` (aktueller Stand: 2089 Szenarien). Die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
