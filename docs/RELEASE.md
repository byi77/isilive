...
`tools/validate_usecases.lua` ist Pflicht fuer das Release-Gate, fuehrt beide Regelvalidatoren zuerst aus und validiert danach die aktuell registrierten Szenarien ueber `tools/usecase_scenarios.lua` (Stand nach StatsBox-, Nameplate-, Locale- und entferntem Pre-Accept-LFG-Invite-Hint-Pfad: 1946 Szenarien). Die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
