...
5. `tools/validate_usecases.lua` fuehrt beide Validatoren zuerst aus und deckt danach die aktuell registrierten Szenarien aus `tools/usecase_scenarios.lua` ab; die exakte Anzahl wird bei jedem Lauf ausgegeben und die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
   ...
   Letzter voller Coverage-Audit-Stand (`2026-07-26`, lokaler Preflight bei 0.9.358): **92.07% Gesamt-Line-Coverage** (`34859 / 37863` Zeilen) bei `2274 passed, 0 failed`. Vorheriger Stand (`2026-07-26`, 0.9.357): 92.07% (`34840 / 37842` Zeilen) bei `2272 passed, 0 failed`. Das Coverage-Gate bleibt bei mindestens 88.00% gesamt und 80.00% pro Produktionsdatei.
...
```text
| isiLive v0.9.358 BETA                                  Open/Close CTRL-F9 [M+][H][V][Gear][L][X]                 |
|------------------------------------------------------------------------------------------------------------------|
