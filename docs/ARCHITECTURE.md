...
5. `tools/validate_usecases.lua` fuehrt beide Validatoren zuerst aus und deckt danach die aktuell registrierten Szenarien aus `tools/usecase_scenarios.lua` ab; die exakte Anzahl wird bei jedem Lauf ausgegeben und die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
   ...
   Letzter voller Coverage-Audit-Stand (`2026-08-01`, lokaler Preflight bei 0.9.361): **92.17% Gesamt-Line-Coverage** (`35674 / 38703` Zeilen) bei `2288 passed, 0 failed`. Vorheriger Stand vor der responsiven Demo-Simulator-Ueberarbeitung: 92.07% (`34897 / 37903` Zeilen) bei `2284 passed, 0 failed`. Das Coverage-Gate bleibt bei mindestens 88.00% gesamt und 80.00% pro Produktionsdatei.
...
```text
| isiLive v0.9.361 BETA                                  Open/Close CTRL-F9 [M+][H][V][Gear][L][X]                 |
|------------------------------------------------------------------------------------------------------------------|
