...
`tools/validate_usecases.lua` is mandatory for release gating, runs both rule validators first, and then validates 419 scenarios across 34 modules. The rule validators currently index 415 deterministic tests.
...
```powershell
git tag isiLive_release_0.9.105
git push origin isiLive_release_0.9.105
```
