...
`tools/validate_usecases.lua` is mandatory for release gating, runs both rule validators first, and then validates 457 scenarios across 34 modules. The rule validators currently index 457 deterministic tests.
...
```powershell
git tag isiLive_release_0.9.114
git push origin isiLive_release_0.9.114
```
