...
![WoW 12.1.0 Midnight Retail](https://img.shields.io/badge/WoW-12.1.0%20Midnight%20Retail-00A2FF?style=for-the-badge)
![isiLive 0.9.400](https://img.shields.io/badge/isiLive-0.9.400-1E90FF?style=for-the-badge)
...
Version `0.9.400` is the pet-stuck voice alert: the moment the game shows the red "No path available for your pet" error, isiLive says "Check your pet!" (German clients: "Achte auf deinen Begleiter!"), in every client language, at most every 5 seconds and outside raids, with its own default-on sound toggle. Version `0.9.399` is an audit release: the main window and the tank/healer marker icons no longer trigger "Interface action failed" in combat -- dragging waits for combat to end, while closing, UI-scale changes and `/isilive resetui` are applied once it does -- a marker icon whose player left mid-fight can no longer mark your current target, an abandoned key is recorded under its own dungeon and level, and a malformed Power Infusion sync message can no longer inject markup into your chat. Version `0.9.398` is a refreshed enemy-forces snapshot: same per-mob counts, new source revision and a recorded lifetime running to 2026-10-02, which is what keeps the database-backed forces displays open. It also marks the last-run DPS work of the previous three releases as confirmed in a live key -- the completion handler records the run itself, with the timed flag and the real keystone level, and every group member resolves. Version `0.9.397` is the last piece of the last-run DPS story, and the one only a live log could show: as a key ends, the game hands out the damage numbers in a protected form. Reading such a value is allowed, comparing it is not -- and the capture compared one, which aborted the whole key-completion handler before a single number was recorded. The log of a key that finished in time shows the completion arriving and the dispatch failing in the very next line; the run was then saved half a minute later by the abandoned-key path, once the numbers were ordinary again outside the instance. Every damage-meter field is now read the protected-safe way, and a value that stays masked is skipped rather than compared. Version `0.9.396` is the other half of the last-run DPS fix: it records a key that ends without being completed -- abandoned, depleted and left, or left mid-run -- is recorded as well. Such a key reports no completion information at all, so the run had nothing to be filed under and the entire group's damage was discarded, even though the damage meter still held every member of the group that had just broken up. The map and keystone level are now remembered when the key starts, and recording a finished key clears that note so a completed run can never be written twice. Version `0.9.395` is the release that actually fixes the last-run DPS column, and it corrects what 0.9.393 claimed to have fixed. The column showed a stale number for you and a dash for everyone else because no completed key had ever been recorded: the call that reads a finished key's map and level was deprecated in 11.0.5 and removed in patch 12.0, and a removed member of a `C_` namespace reads as nothing rather than failing loudly -- the guarded call simply reported "unavailable" and the run never reached the damage meter. It is replaced by the table-returning call Blizzard kept. Two things around it are fixed as well: a dungeon still reports the keystone difficulty after the timer stops, so every finished key opened a second, phantom run that was written over the key's own result on the way out; and a capture that found nothing used to erase the previous one and mark every member as missing, which emptied the whole column for the rest of the session. The per-run RIO delta now survives leaving the group, the way the name, item level and rating on those rows already did. For the next time something goes quiet, `/isilive dpsdump` prints what the capture sees at each stage, and the runtime log no longer loses a key's entries to the group finder, whose search results had been filling the entire log buffer within seconds. Version `0.9.394` is the release that lets you pick the interface font. The Display settings now offer the fonts the WoW client ships -- Friz Quadrata, Arial Narrow, Morpheus and Skurri -- so isiLive can sit inside a UI that has been styled elsewhere. Two things stay above that choice: a language that ships its own font keeps it, and a Cyrillic player name switches to a Cyrillic-capable font even under a Latin-only pick -- otherwise a russian realm name would render as boxes. The default is the font every previous version used, so an existing install looks unchanged until it picks something, and a selection whose font disappeared falls back to that default instead of erroring. Switching takes effect immediately, without a reload. Version `0.9.393` is the fix for the DPS column emptying itself after a key. Finishing a key and then leaving the group left a single dash in every row, the player's own included. Two things had to meet for that: a dungeon keeps reporting the keystone difficulty after the timer stops, while the active-key lookup goes empty the moment the key ends, so the tracker for keyless mythic runs treated the finished dungeon as a fresh run of its own -- and closing that phantom run on the way out recorded it over the key's numbers with whatever the damage meter still had, which outside the instance is nothing. A run that captures nothing now leaves the last captured run alone, and an inserted keystone no longer opens a keyless run at all. Version `0.9.392` is a fix release for three visible problems. Your own deaths were undercounted in the roster: the skull marker beside your name stayed at a single death for a whole key, because `UNIT_HEALTH` was the only signal the death watch had and it carries no guaranteed sample while you are dead or running back as a ghost -- a missed sample either dropped the death or left the dead flag latched, and every further own death was then swallowed. The own-death edges now come from `PLAYER_DEAD` and `PLAYER_UNGHOST` as well, every party slot is re-checked whenever the game itself registers a death, and the count is shown from the first death on instead of only from the second. The ready-check row color stopped behind the Kick column, because the tint was pinned to the row's hover area, which deliberately ends there so the management buttons on the right stay clickable; the color now fills the row to the frame edge while the click area is unchanged. And moving the UI-scale slider shifted the whole window, since anchor offsets are measured in the frame's own scale -- they are converted along with the scale now, and the demo simulator tablet docks correctly at scales other than 100% as well. Version `0.9.391` is the second audit follow-up and is localization polish throughout. German was the only language in the addon still written with the ASCII substitutes for its umlauts -- "verfuegbar", "zuruecksetzen", "Schriftgroesse", "Gegnerkraefte" -- across settings labels, the slash-command help and the reset dialogs; 110 strings are corrected, and words that merely look transliterated (Auerochse, Abschluss, Klasse) keep their spelling. The shared LFG group-bonus description had the same problem in five languages at once, worst in Turkish, where the dotted and dotless i are separate letters and "yararli sinif bufflari icin yesil" was simply misspelled. Alongside that, the button-length gate now also covers the demo simulator's action titles, which had never been checked and had let a 25-character Italian label through. Version `0.9.390` is an audit follow-up with no new features. Six languages were still showing English text in around 105 places each -- the whole demo simulator, the `/isilive` help output, the dungeon- and raid-entered notices, the accepted-invite notices, the teleport tooltips and the LFG group-bonus labels -- because the build gate compares locale key sets and format placeholders but never the text itself, so an untranslated copy of the English string passes it unnoticed; French, Spanish, Portuguese, Italian, Russian and Turkish are translated now, with dungeon names, the season title and the spell name Power Infusion deliberately left in English. Two roster role-icon problems are fixed alongside it: a damage dealer's role icon carried no marker macro yet still sat above its row and swallowed every click in its own corner, which is where the right-click whisper that works everywhere else in the row went; and a group member who died mid-fight kept a clickable role button, because secure attributes cannot be rewritten during combat and the only check was on the macro text, which stays identical when the player behind it has not changed. Version `0.9.389` is the rollback of the M+ mockup redesign. It restores the previous UI colors, typography, language flags and timer layout, and removes the development mockup. The independent VIP Unholy DK fixes from 0.9.388 remain in place. Version `0.9.388` is the release that brings the VIP Unholy Death Knight helper in line with patch 12.1. Soul Reaper no longer consumes Putrefy charges, so the two buttons the feature warns about stopped sharing a resource — which was the assumption the Putrefy warning rested on. That warning now reads its own charges and stays quiet while you hold two or more, where banking one would only run the 30-second recharge into its cap. The warning window itself was a fixed 30-second wait that only lined up while Dark Transformation sat at exactly 45 seconds; it now follows the live cooldown and always covers the last 15 seconds before it returns. The demo simulator gained a VIP DK warning action that skips the cooldown wait and nothing else — every guard still runs — so the feature can be checked outside a real fight. Version `0.9.387` is a fix release for seven findings from two audit rounds. Power Infusion is announced exactly once when you receive it: your own client sees the buff and the casting priest's addon message arrives moments later, and only the local paths knew about each other, so the line, the sound and the center alert came twice. Matching those two reports is harder than it sounds, because 12.1 hides the caster from your client inside instances and the two sides spell names differently — the fix keeps priests from different realms apart while still recognising the same cast under two different names, and announces rather than staying silent when it cannot tell. The per-run RIO delta now appears only when the refresh behind it actually delivered, instead of also after every attempt failed or after you left the group. The enemy-forces tracker no longer replaces a confirmed percentage with a synthetic 0% when 12.1 masks the count, keeps its count, total and percentage consistent with each other, and a quick second pull keeps its own numbers instead of being cleared by the previous pull's timer. A corrupted saved-settings version stamp can no longer stall the login with a near-endless migration loop. And a release tag that names a different version than the addon it packages is now refused before anything is built. Version `0.9.386` is a fix release for two holes that never show on screen: one in the raid transition, one in the release gate. Leaving a raid lifts the reduced raid profile immediately but defers the event re-registration by one tick, because the client forbids it inside a protected dispatch — so a raid re-entered within the same burst of roster updates re-armed exactly the per-member event traffic that profile exists to remove. Separately, a manually dispatched pre-release ran its quality gate against the branch selected in the Actions interface instead of the tag it was about to package, so a green gate did not prove the shipped commit green. The release notes that go to CurseForge had been carried forward by version number alone since 0.9.373 and now describe what actually shipped. Version `0.9.385` is the fix for the isiLive shortcuts disappearing from the ESC menu after a reload inside a Mythic+ key, until the client was restarted. Secure buttons cannot be changed during combat, nor while a key is running -- so a reload in a dungeon always built that panel in a blocked state. The refresh was scheduled for later but discarded twice: once by the drain that removed it before trying, once by the forced apply that cleared the retry its own inner calls had just re-armed. The scheduled refresh now survives until it actually runs, and the shortcuts return by themselves the next time nothing blocks them. Version `0.9.384` is a fix release for four things. Power Infusion is announced again when you receive it: the tracker depended on two aura fields that WoW 12.1 masks inside instances, and now asks Blizzard directly whether you carry the buff instead. The portal navigator no longer clips its outer slots, whose labels fell outside the bottom border once Season 2 left those positions empty. The enemy-forces nameplate lost its background plate and shows the percentage directly on the nameplate. And the per-mob refresh in a key is coalesced, because the masked mob GUID forces every nameplate through the more expensive progress API on every scenario tick. Version `0.9.383` is the release that activates Midnight Season 2 and drops WoW 12.0.7. Blizzard shipped Season 2 and the runtime had already switched to it, but the manifest still named Season 1 as active — which is what the build gates and the fallback read, so the enemy-forces database stayed pinned to a season nobody plays. That also explains why mob percentages had vanished on S2 keys: the S2 entry declared no MDT directory and no forces requirement, so the generator skipped the season entirely. MDT 6.2.15 ships the S2 dungeons in the same directory as the S1 ones; that is recorded now, and the database is generated from it with all eight dungeons. Season 1 remains fully usable as the manual fallback, without MDT mob percentages. The addon now declares WoW 12.1.0 only. Version `0.9.382` is the release that finally solves what three earlier attempts could not: the ESC menu now closes itself after you click a travel or mount shortcut, so the cast keeps running and there is no ESC press left to cancel it. Blizzard's ESC chain cannot be intervened in from an addon without breaking its own protected calls, so the reason to press the key is removed instead. The hook runs after the click rather than before it, because these buttons live inside the game menu and pulling that frame away mid-click is not safe. During combat the menu stays open, where hiding it is not permitted. Version `0.9.381` is the release that withdraws the ESC cast guard for good and takes a per-mouse-move cost out of the unit tooltip. The guard could not work: WoW walks its ESC handlers inside one loop, and while each call is isolated the loop stays tainted by whatever ran in it before — so a handler placed ahead of the cast-cancel step made WoW's own cast cancellation fail with a forbidden-action error on every ESC press. It is removed, along with its tests, and a test now keeps isiLive out of that chain. In practice this means ESC once again cancels a hearthstone or mount cast started from the ESC panel; the original request is unfulfilled rather than fixed. Separately, the server-language flag in the unit tooltip ran a realm lookup for every NPC the mouse crossed, because the world-cursor tooltip refreshes continuously and creatures pass the player check — enough of them at once and the client aborted with "script ran too long". Non-player GUIDs are now rejected before any work happens. Version `0.9.380` is a hotfix for the ESC key, which 0.9.379 could disable completely. The cast guard added in that release could raise instead of answering, and WoW runs its ESC handlers in a plain loop that does not catch errors — so a raising handler aborted the whole chain, and ESC stopped opening the game menu and stopped closing any open window, for every addon. The guard compared two values before checking whether they were masked, which is why this appeared in combat and inside instances and stayed invisible outside. Both reads are now checked first, and the handler can no longer let an error escape at all. What 0.9.379 intended is unchanged: with the game menu open and a cast in flight, ESC closes only the menu. Version `0.9.379` is a fix release for four visible problems. The roster's RIO column clipped four-digit scores to `(+168)13…` because it was one digit too narrow; it is wider now, and the space comes from the oversized Kick column so no other column moves. The card shown after an accepted invite read "Unknown dungeon" whenever that dungeon had no teleport of its own — and for every raid invite, which never has one — because the name was taken from the teleport lookup alone; it now falls back to the season data. Pressing ESC to close the game menu after starting a hearthstone or a mount from the ESC panel cancelled the cast: WoW reordered its ESC handling so cast cancellation runs before open panels are closed, and isiLive now claims the key back for exactly that case and closes only the menu. And the server-language flag in the unit tooltip kept its state on Blizzard's own tooltip frame, which tainted it and produced "attempt to compare a secret number value" errors reported from unrelated addons; that state now lives outside the frame. Version `0.9.378` fixes a WoW 12.1 regression that spammed the chat with error lines inside Mythic+ keys and raids. Patch 12.1 started masking an aura-update flag as a protected value; reading it crashed the aura handler on every update, and because the Power Infusion tracker runs first in that handler, the crash also silently disabled the Bloodlust/Sated countdown and the Bloodlust button warning for the whole run. All three are restored, and repeated dispatch errors can no longer flood the chat. Version `0.9.377` is a build-gate performance fix; nothing changes on screen. The check that proves every English locale key is still referenced in production code scanned the entire codebase once per key, taking around 33 seconds; it now builds a single lookup index and answers in about 0.2 seconds with an identical result.

**Active season:** `midnight_s2` with 8 supported dungeons: `KR`, `TOS`, `RLP`, `TBV`, `VA`, `DON`, `MR`, `AOF` (the German client abbreviates six of them differently: `TVS`, `RLB`, `DBT`, `ADL`, `NB`, `MG`, `ADF`)

**Previous season:** `midnight_s1` stays fully recorded and manually selectable as the fallback when Blizzard's challenge-map table and the manifest ever disagree. It declares no forces requirement, because the generated MDT database tracks whichever season is active; S1 therefore works completely except for MDT mob percentages. Season selection remains an exact match against Blizzard's challenge-map table at runtime.
