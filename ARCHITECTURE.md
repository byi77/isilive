...
22. CdTracker lust onset detection must combine player harmful-aura scanning with direct local lust spellcast signals, normalize aura `spellId` values safely via `tonumber(...)` before lookup, and zone/world transition suppression must not treat a continuing lust aura as a new onset.
...
5. `tools/validate_usecases.lua` runs both validators first and then covers 417 deterministic tests indexed and 424 scenarios across 37 modules.

## UI Structure (ASCII Sketch)

```text
| isiLive                                                                          V.0.9.97 [H][V][M][X]|
|---------------------------------------------------------------------------------------------------|
...
| CdTracker | Battle-res charges, `tonumber(...)`-normalized harmful lust-aura scans, and local lust spellcasts | Live BRes charges/cooldown and Lust countdown row state with zone-transition-safe onset suppression |
